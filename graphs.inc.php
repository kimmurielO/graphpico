...
function getGraphParentTemplates(array $graphs, $flag) {
	...
	foreach ($data['links'] as &$parent_graph) {
		...
		if(array_key_exists($parent_graph['graphid'], $hostids)){
			$parent_graph['hostid'] = $hostids[$parent_graph['graphid']];
		}
		else{
			$parent_graph['hostid'] = 0;
		}
		...
		if ($flag == ZBX_FLAG_DISCOVERY_PROTOTYPE) {
			if(array_key_exists($parent_graph['graphid'], $lld_ruleids)){
				$parent_graph['lld_ruleid'] = $lld_ruleids[$parent_graph['graphid']];
			}
			else{
				$parent_graph['lld_ruleid'] = 0;
			}
		}
		/*if ($flag == ZBX_FLAG_DISCOVERY_PROTOTYPE) {
...
*/
	...
	foreach ($data['templates'] as $hostid => &$template) {
		if(array_key_exists($hostid, $db_templates)){
			$template = [
				'hostid' => $hostid,
...
function getSameGraphItemsForHost($gitems, $destinationHostId, $error = true, array $flags = []) {
	...
	if($flags){
		$flagsSql = ' AND '.dbConditionInt('dest.flags', $flags);
	}
	else {
		$flagsSql = '';
	}
